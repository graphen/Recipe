...
public class RecipeController {
    ...
    public String saveRecipe(@Valid @ModelAttribute("recipe") RecipeCommand recipeCommand, 
        ...
        if(bindingResult.hasErrors()) {
            model.addAttribute("action", recipeCommand.getId() != null ? "Edit" : "Create");
            addFormAttributes(model);
            return "recipes/form";
        }
        ...
        try {
        	if (recipeCommand.getId() != null) {
                // Preserve existing image for editing
    ...
    @GetMapping("/{id}/image")
    public ResponseEntity<byte[]> renderRecipeImage(@PathVariable Long id) {
        RecipeCommand recipeCommand = recipeService.findRecipeById(id);
